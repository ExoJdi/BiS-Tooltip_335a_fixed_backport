...
local function createItemFrame(item_id, size, with_checkmark)
    ...
    -- Horde->Alliance item translation (only for Horde players)
    if isHorde and Bistooltip_horde_to_ali then
        local aliItemID = Bistooltip_horde_to_ali[item_id]
        if aliItemID then
            item_id = aliItemID
        end
    end
    ...
    if not itemName then
        ...
        -- Basic interactivity even before the item is cached
...
local function mergeOldPhases()
    if type(Bistooltip_wowtbc_bislists_old) ~= "table" then return end
    if type(Bistooltip_wowtbc_bislists) ~= "table" then return end

    for className, classTbl in pairs(Bistooltip_wowtbc_bislists_old) do
        if type(classTbl) == "table" then
            Bistooltip_wowtbc_bislists[className] = Bistooltip_wowtbc_bislists[className] or {}
            for specName, specTbl in pairs(classTbl) do
                if type(specTbl) == "table" then
                    Bistooltip_wowtbc_bislists[className][specName] = Bistooltip_wowtbc_bislists[className][specName] or {}
                    for phaseName, phaseTbl in pairs(specTbl) do
                        -- Only merge missing phases (T7/T8 live here)
                        if Bistooltip_wowtbc_bislists[className][specName][phaseName] == nil then
                            Bistooltip_wowtbc_bislists[className][specName][phaseName] = phaseTbl
                        end
                    end
                end
            end
    ...
    -- drop root ref; nested tables are now referenced from main table
    Bistooltip_wowtbc_bislists_old = nil
end
...
function BistooltipAddon:createMainFrame()
    ...
    drawSpecData()
        ...
        -- Bottom controls (Reload Data)
    local buttonContainer = AceGUI:Create("SimpleGroup")
    buttonContainer:SetFullWidth(true)
    buttonContainer:SetLayout("List")
    buttonContainer:SetAutoAdjustHeight(false)
    buttonContainer:SetHeight(32) -- 24 button + 4px top + 4px bottom

    local topPad = AceGUI:Create("Label")
    topPad:SetText("")
    topPad:SetFullWidth(true)
    topPad:SetHeight(4)
    buttonContainer:AddChild(topPad)
